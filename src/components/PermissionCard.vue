            ...
            <q-card class="permission-card col q-mt-md" flat>
                <q-card-section horizontal>
                    <q-card-section class="permission-name-section row items-center justify-center">
                        <div class="text-bold">
                            {{ `${permissionLocal.perm_name} (${permissionLocal.required_auth.threshold})` }}
                        </div>
                    </q-card-section>
                    <q-card-section class="permission-key-section">
                        ...
                        <div v-for="a in permissionLocal.required_auth.accounts"
                             :key="`${a.permission.actor}-${a.permission.permission}`"
                        >
                            <div><span>{{ `+${a.weight} &nbsp; &nbsp; ` }}</span><span class="text-bold"
                                                                                       v-html="formatAccount(a.permission.actor?.toString(), 'account')"
                            ></span><span> @{{ a.permission.permission }}</span></div>
                        </div>
                        <div v-for="w in permissionLocal.required_auth.waits" :key="`${w.wait_sec}-${w.weight}`">
                            <div><span>{{ `+${w.weight} &nbsp; &nbsp; ` }}</span><span>{{
                                    formatWait(w.wait_sec.value)
                                }}</span></div>
                        </div>
                    </q-card-section>
                    <q-card-section v-if="permissionLocal.permission_links.length > 0"
                                    class="permission-action-section"
                    >
                        <div v-for="link in permissionLocal.permission_links" :key="link.action">
                            {{ link.code }}::{{ link.action }}
                        </div>
                    </q-card-section>
                </q-card-section>
            </q-card>
