<template>
    <q-dialog ref="walletDialog" class="modal-container">
        ...
        <q-list>
            ...
            >
                <q-item-section class="cursor-pointer" avatar @click="onLogin(idx)"><img :src="wallet.getStyle().icon"
                                                                                         width="30"
                ></q-item-section>
                <q-item-section class="cursor-pointer" @click="onLogin(idx)">{{ wallet.getStyle().text }}
                </q-item-section>
                <q-item-section class="flex" avatar>
                    <q-spinner v-if="loading === wallet.getStyle().text" :color="wallet.getStyle().textColor"
                               size="2em"
                    />
                    <q-btn
                        v-else
