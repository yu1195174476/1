...
<template>
    ...
    <div class="relative-position" :class="showOverflow ? '' : 'overflow-hidden'"
    ...
    >
        ...
        <AccountFormat :account="transferData.to" type="account">&nbsp;</AccountFormat></span><span class="text-bold"
            >{{ ' ' + transferData.quantity }}</span></div>
            <div class="col-12">
                <div class="text-weight-bold">memo:&nbsp;<span class="text-weight-regular" v-if="transferData.memo"
                >{{ transferData.memo }}</span><span class="text-weight-regular" v-else>n/a</span></div>
            </div>
        </div>
        <div class="row" v-else ref="dataBox">
            <div class="col-12" v-for="val in formatGeneralData(data)" :key="val.key">
                <div class="text-weight-bold">{{ val.key }} :<span class="text-weight-regular"
                                                                   v-if="isAccount(val.key)"
                >
          <AccountFormat :account="val.value" type="account">&nbsp;</AccountFormat></span><span
                    class="text-weight-regular" v-else
                >{{ val.value }} &nbsp;</span></div>
            </div>
        </div>
        <q-btn v-if="isOverflowing" flat size="xs" :icon="showOverflow ? 'expand_less' : 'expand_more'"
